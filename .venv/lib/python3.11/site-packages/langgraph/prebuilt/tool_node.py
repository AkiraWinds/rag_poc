    ...
    def _inject_tool_args(
        ...
        if injected.state:
            ...
            if isinstance(state, list):
                collected_fields = list(injected.state.values())
                if (
                    len(collected_fields) == 1
                    and collected_fields[0] == self._messages_key
                ) or collected_fields[0] is None:
                    ...
                    if any(state_field for state_field in injected.state.values()):
                        collected_fields_str = ", ".join(f for f in collected_fields if f)
                        err_msg += (
                            f" State should contain fields {collected_fields_str}."
                        )
