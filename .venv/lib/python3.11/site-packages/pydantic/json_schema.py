...
class GenerateJsonSchema:
    ...
    def typed_dict_schema(self, schema: core_schema.TypedDictSchema) -> JsonSchemaValue:
        ...
        named_collected_fields: list[tuple[str, bool, CoreSchemaField]] = [
            (name, self.field_is_required(field, total), field)
            for name, field in schema['fields'].items()
            if self.field_is_present(field)
        ]
        if self.mode == 'serialization':
            named_collected_fields.extend(self._name_required_computed_fields(schema.get('computed_fields', [])))
        cls = schema.get('cls')
        config = _get_typed_dict_config(cls)
        with self._config_wrapper_stack.push(config):
            json_schema = self._named_collected_fields_schema(named_collected_fields)
    ...
    def _named_collected_fields_schema(
        self, named_collected_fields: Sequence[tuple[str, bool, CoreSchemaField]]
    ) -> JsonSchemaValue:
        properties: dict[str, JsonSchemaValue] = {}
        collected_fields: list[str] = []
        for name, required, field in named_collected_fields:
            if self.by_alias:
        ...
        if collected_fields:
            json_schema['required'] = collected_fields
        return json_schema
    ...
    def dataclass_args_schema(self, schema: core_schema.DataclassArgsSchema) -> JsonSchemaValue:
        ...
        if self.mode == 'serialization':
            named_collected_fields.extend(self._name_required_computed_fields(schema.get('computed_fields', [])))
        return self._named_collected_fields_schema(named_collected_fields)
...
class WithJsonSchema:
    ...
    def __get_pydantic_json_schema__(
        ...
        if self.json_schema is None:
            # This exception is handled in pydantic.json_schema.GenerateJsonSchema._named_collected_fields_schema
            raise PydanticOmit
        else:
